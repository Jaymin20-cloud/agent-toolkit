...
def _is_slim_deploy() -> bool:
    """Small installs (e.g. Vercel): only the chatbot agent; set AGENT_TOOLKIT_SLIM=1."""
    return os.environ.get("AGENT_TOOLKIT_SLIM", "").lower() in ("1", "true", "yes")


def _make_agents() -> tuple[dict[str, Agent], str]:
    if _is_slim_deploy():
...
agents, DEFAULT_AGENT = _make_agents()
